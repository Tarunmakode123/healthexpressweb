export const SERVICES_DATA = [
  {
    id: 'diagnostics',
    slug: 'diagnostics',
    title: 'Diagnostics',
    shortDesc: 'Blood tests and health checkups',
    description: 'Comprehensive diagnostic testing including blood profiles, pathology, metabolic panels, and routine health checkups delivered at home or partner labs.',
    iconName: 'FlaskConical',
    badge: 'Most Popular',
    features: ['Home sample collection', 'NABL accredited partner labs', 'Digital report within 24 hours', 'Doctor consultation on report']
  },
  {
    id: 'imaging',
    slug: 'imaging',
    title: 'Imaging',
    shortDesc: 'X-rays, MRI, CT and other imaging services',
    description: 'High-precision diagnostic imaging at top accredited centers near you. Fast appointment slot confirmation and digital DICOM/radiology reporting.',
    iconName: 'Camera',
    badge: 'Advanced Diagnostic',
    features: ['MRI & CT scans', 'Digital X-Rays & Ultrasound', 'Mammography & PET-CT', 'Prioritized scheduling']
  },
  {
    id: 'home-healthcare',
    slug: 'home-healthcare',
    title: 'Home Healthcare',
    shortDesc: 'Nursing, caregiving and more at home',
    description: 'Professional nursing care, post-surgical rehabilitation, elderly care, and specialized medical assistance in the comfort of your home.',
    iconName: 'Home',
    badge: 'At Home Care',
    features: ['Certified nurses & caregivers', 'Post-hospitalization care', 'IV therapy & dressing', 'Regular vital monitoring']
  },
  {
    id: 'telemedicine',
    slug: 'telemedicine',
    title: 'Telemedicine',
    shortDesc: 'Consult with healthcare professionals',
    description: 'Instant and scheduled video consultations with verified general physicians, specialists, and superspecialists anytime.',
    iconName: 'Video',
    badge: 'Instant Connect',
    features: ['50+ Specialties available', 'Digital prescription issue', 'Follow-up coordination', '100% Private & Secure']
  },
  {
    id: 'pharmacy',
    slug: 'pharmacy',
    title: 'Pharmacy',
    shortDesc: 'Medicines through partner pharmacies',
    description: 'Fulfillment of genuine prescription medicines and health supplies delivered directly to your doorstep through verified retail partner pharmacies.',
    iconName: 'Pill',
    badge: 'Doorstep Delivery',
    features: ['100% Genuine medicines', 'Prescription validation', 'Temperature-controlled delivery', 'Refill reminders']
  },
  {
    id: 'genetic-testing',
    slug: 'genetic-testing',
    title: 'Genetic Testing',
    shortDesc: 'Access genetic testing services',
    description: 'Advanced DNA sequencing, hereditary risk screening, nutrigenomics, and carrier screening with professional genetic counseling.',
    iconName: 'Dna',
    badge: 'Precision Medicine',
    features: ['Hereditary disease risk', 'Pharmacogenomics', 'Genetic counselor review', 'Saliva / Blood collection']
  },
  {
    id: 'preventive-healthcare',
    slug: 'preventive-healthcare',
    title: 'Preventive Healthcare',
    shortDesc: 'Screening, checkups and ongoing health awareness',
    description: 'Proactive wellness packages designed to detect early indicators of lifestyle diseases, heart health risks, diabetes, and organ wellness.',
    iconName: 'HeartPulse',
    badge: 'Proactive Care',
    features: ['Age-tailored packages', 'Comprehensive body profiling', 'Lifestyle coaching', 'Annual wellness tracker']
  }
];
